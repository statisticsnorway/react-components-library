...
class App extends Component {
  ...
  handleUploadInput(event, ref) {
    ...
    for(let i = 0, l = length; i < l; i++) {
      ...
      fileReader.onloadend = () => {
        ...
        } else if(ref === 'defaultUiSchemas'){
          this.setState({
            ...this.state,
            defaultUiSchema: JSON.stringify(json, null, 2),
            defaultUiSchemasReady: true
          })
  ...
  handleMergeButtonClick = (event) => {
    ...
    if(event.target.name === 'mergeUiSchema'){
      let temp = mergeUiSchemas(this.state.mergedDefaultUiJsonSchema, this.state.uiSchemaFiles)
      setTimeout(() => {
        this.setState({
          ...this.state,
          mergedUiJsonSchema: JSON.stringify(temp, null, 2),
  ...
  render() {
    const {jsonSchemasReady, defaultUiSchemasReady, uiSchemasReady, mergedDefaultUiSchemaReady} = this.state
    return (
      <div>
        <Step.Group widths={3}>
          ...
          <Step link ref="defaultUiSchema" completed={defaultUiSchemasReady}>
            <Icon name='file text outline' />
            <Step.Content>
              <Step.Title></Step.Title>
              <Step.Description></Step.Description>
              <Button name='defaultUiSchemaUploader' color='grey' content='Velg Default UI Schema'
                      onClick={(event) => {
                        this.uploadButtonClick(event)
                      }} />
              <input ref='defaultUiSchemaUploader' type='file' multiple style={{display: 'none'}} accept='.json'
                     onChange={(event, name = 'defaultUiSchemas') => {
                       this.handleUploadInput(event, name)
                     }} />
            </Step.Content>
          </Step>
              ...
              <Grid.Column>
                <Button.Group floated='left'>
                  ...
                  <Button name="mergeDefaultUI"
                          ...
                          disabled={!(jsonSchemasReady && defaultUiSchemasReady)}
                  />
                </Button.Group>
              </Grid.Column>
              <Grid.Column>
                <Button.Group floated='left'>
                  ...
                  <Button name="mergeUiSchema"
                          ...
                          disabled={!(jsonSchemasReady && defaultUiSchemasReady && mergedDefaultUiSchemaReady)}
                  />
                </Button.Group>
              </Grid.Column>
            </Grid.Row>
